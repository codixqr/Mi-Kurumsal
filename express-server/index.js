...
app.get("/api/admin/seed", authMiddleware, async (req, res) => {
  try {
    ...
    return res.json({ success: true, message: "Tüm örnek veriler başarıyla yüklendi. Sayfaları yenileyebilirsiniz." });
  } catch (err) {
...
app.get("/api/admin/seed-legacy", authMiddleware, async (req, res) => {
  try {
    const pg = pool;
...
app.get("/api/reports/summary", authMiddleware, async (req, res) => {
  try {
    ...
    const [leads, wins, projects, contracts, tasks, topSectorR, brands, locations, financeR] = await Promise.all([
      pool.query(`SELECT COUNT(*)::int AS c FROM investors WHERE deleted_at IS NULL AND created_at BETWEEN $1 AND $2`, [dateFrom, dateTo]),
      pool.query(`SELECT COUNT(*)::int AS c FROM investors WHERE deleted_at IS NULL AND pipeline_stage ILIKE '%Kapandı%' AND created_at BETWEEN $1 AND $2`, [dateFrom, dateTo]),
      pool.query(`SELECT COUNT(*)::int AS c FROM projects WHERE deleted_at IS NULL AND created_at BETWEEN $1 AND $2`, [dateFrom, dateTo]),
      pool.query(`SELECT COUNT(*)::int AS c FROM contracts WHERE deleted_at IS NULL AND created_at BETWEEN $1 AND $2`, [dateFrom, dateTo]),
      pool.query(`SELECT COUNT(*)::int AS c FROM tasks WHERE deleted_at IS NULL AND status != 'Tamamlandı'`),
      pool.query(`SELECT sector, COUNT(*)::int AS c FROM investors WHERE deleted_at IS NULL GROUP BY sector ORDER BY c DESC LIMIT 1`),
      pool.query(`SELECT COUNT(*)::int AS c FROM brands WHERE deleted_at IS NULL`),
      pool.query(`SELECT COUNT(*)::int AS c FROM locations WHERE deleted_at IS NULL`),
      pool.query(`SELECT COALESCE(SUM(amount::numeric),0)::numeric AS s FROM finance_records WHERE created_at BETWEEN $1 AND $2`, [dateFrom, dateTo]),
    ]);
